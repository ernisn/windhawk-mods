...
- Duplicate property entries will be deduplicated automatically (only the first occurrence is kept).
...
static void LoadSettings() {
    ...
    for (int i = 0; ; i++) {
        ...
        // Wh_GetStringSetting returns nullptr if the index is out of range, or an
        // empty string if the field exists but is blank.
        if (!rawProp) {
            break;
        }
        if (rawProp[0] == L'\0') {
        ...
        if (SUCCEEDED(PSGetPropertyKeyFromName(rawProp, &key))) {
            // Deduplicate: skip if this key is already in newColumns.
    ...
    // Decide what to do based on what we read:
    //   1. We got at least one valid column -> commit the new list.
    //   2. We saw entries but none were valid (all unrecognised/duplicates) ->
    //      commit empty; the apply path will skip rather than fall back to hardcoded defaults.
    //   3. We saw nothing at all -> settings system likely not ready yet, or
    //      user truly has zero columns. Either way, do NOT overwrite the in-memory
    //      list with hardcoded defaults; that was the v1.1 bug where a system /
    //      Explorer restart could race the settings load and clobber user config.
    if (!newColumns.empty()) {
        ...
        // User config exists but every entry was invalid. Honour that intent and
        // apply nothing rather than silently substituting defaults.
        g_columns.clear();
        g_settingsLoaded = true;
        ...
        // No entries readable. Could be: (a) settings not ready yet, or (b) user
        // emptied the list entirely. We can't tell the difference from here, so
        // we play it safe: leave g_columns and g_settingsLoaded untouched. If this
        // is the very first load, g_settingsLoaded stays false and apply paths
        // will skip until SettingsChanged fires with real data.
...
static void ApplyForcedColumns(void* pThis) {
    // Guard against ever applying when settings haven't been successfully loaded.
    // This is the core fix for the post-restart "default columns" bug.
    ...
    if (FAILED(hrWnd)) {
        ...
        // Continue anyway; GetWindowDpi will fall back to system DPI.
    }
    ...
    for (const auto& col : g_columns) {
        ...
        if (SUCCEEDED(pCM->GetColumnInfo(col.key, &ci))) {
            ...
            // Use a tolerance to avoid re-applying on 1px discrepancies caused by
            // rounding or system-side adjustments. abs() on UINT requires a cast.
...
void Wh_ModAfterInit() {
    ...
    // If settings weren't ready during ModInit, try one more time here. By the
    // time AfterInit runs the settings layer is more likely to be initialised.
    if (!g_settingsLoaded) {
